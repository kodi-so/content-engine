...
const toolDefinitions = [
  ...
  agentRuntimeTool({
    ...
    description: "Create an inactive recurring content automation from a brief, pillars, schedule, approval mode, and defaults.",
    plannerGuidance: [
      "When the user wants recurring, scheduled, or autopilot posting, propose an automation: draft the brief and pillars from the conversation, confirm the schedule and approval mode, then call automation.create.",
      "To turn the current conversation's output style into a recurring series, create an automation whose brief describes what was just made.",
    ],
    category: "automation",
    inputSchema: jsonSchema("Automation create request.", {
      name: stringSchema("Automation name."),
      brief: stringSchema("Natural-language theme, voice, audience, do/don't guardrails, and desired output."),
      pillars: stringArraySchema("Content pillars or recurring topic buckets."),
      formatMix: stringSchema("Optional format mix such as mostly slideshows, occasional video."),
      schedule: looseObjectSchema("Structured schedule with timezone and postingTimes [{dayOfWeek,hour,minute}]."),
      approvalMode: stringSchema("Approval mode.", ["auto_publish", "require_approval"]),
      socialAccountIds: stringArraySchema("Social account ids to post to."),
      generationDefaults: looseObjectSchema("Optional defaults: imageResolution, aspectRatio, imageModel, videoModel."),
      budget: looseObjectSchema("Optional budget guardrails: maxUsdPerRun, maxUsdPerMonth."),
    }),
    outputSchema: fieldsSchema("Automation result.", {
    ...
    confirmation: {
      required: true,
      risk: "low",
      reason: "Automations can later run unattended, though new automations are created inactive.",
    },
    checkpoint: {
      behavior: "before",
      defaultInDebugMode: false,
      label: "Create automation",
      description: "Create an inactive recurring automation for review.",
    },
    artifactBehavior: noArtifacts,
  }),
  agentRuntimeTool({
    ...
    description: "Patch an existing automation by id or name.",
    plannerGuidance: [
      "When the user references an existing automation, use automation.update rather than creating a new one.",
    ],
    category: "automation",
    inputSchema: jsonSchema("Automation update request.", {
      automationId: stringSchema("Automation id. Use when known."),
      name: stringSchema("Automation name. Use when id is not known."),
      brief: stringSchema("Updated brief."),
      pillars: stringArraySchema("Updated content pillars."),
      formatMix: stringSchema("Updated format mix."),
      schedule: looseObjectSchema("Updated schedule with timezone and postingTimes."),
      approvalMode: stringSchema("Approval mode.", ["auto_publish", "require_approval"]),
      generationDefaults: looseObjectSchema("Updated generation defaults."),
      budget: looseObjectSchema("Updated budget guardrails."),
    }),
    outputSchema: fieldsSchema("Automation update result.", {
      automationId: "Updated automation id.",
  ...
  agentRuntimeTool({
    ...
    description: "List automation descriptors for planner context.",
    category: "automation",
    inputSchema: emptySchema("No input is required."),
    outputSchema: fieldsSchema("Automation list result.", {
      automations: "Automation descriptors with schedule, status, and defaults.",
    }),
    confirmation: defaultConfirmation,
    checkpoint: noCheckpoint,
    artifactBehavior: noArtifacts,
