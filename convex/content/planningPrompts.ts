...
export function buildOverlayPlannerPrompt(args: PromptArgs): string {
  return [
    ...
    "- Write each textBlock.text as final human-facing on-screen copy, not as renderer metadata. Do not rely on generic prefixes, labels, or template words unless they naturally fit that exact slide.",
    "- When adapting a reference, preserve the role of a text element (hook, label, cue, proof, CTA) rather than copying literal label words that no longer fit the new content.",
    "- Check the planned slide copy for contextual coherence: every label, prefix, and short phrase should make sense when read by itself on the finished slide.",
